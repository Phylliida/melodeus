...
class AsyncSTTStreamer:
    ...
    def __init__(self, config: STTConfig, speakers_config=None):
        ...
        # Deepgram client
        self.deepgram = DeepgramClient(api_key=config.api_key)
        ...
        self._listener_future: Optional[asyncio.Future] = None
        self._connection_ready_event: Optional[asyncio.Event] = None
    ...
    def _bool_to_str(self, value: bool) -> str:
        """Convert boolean flags to lowercase string expected by Deepgram query params."""
    ...
    def _build_listen_params(self) -> Dict[str, Any]:
        ...
        params: Dict[str, Any] = {
            ...
            "smart_format": self._bool_to_str(self.config.smart_format),
            "interim_results": self._bool_to_str(self.config.interim_results),
            "punctuate": self._bool_to_str(self.config.punctuate),
            "diarize": self._bool_to_str(self.config.diarize),
            "vad_events": self._bool_to_str(self.config.vad_events),
        }
    ...
    async def _establish_connection(self) -> bool:
        """Initialize the Deepgram Listen v1 websocket connection."""
        ...
        try:
            self._connection_context = self.deepgram.listen.v1.connect(**params)
            self.connection = self._connection_context.__enter__()
        except Exception as exc:
            print(f"❌ Failed to open Deepgram websocket: {exc}")
            self.connection = None
            self._connection_context = None
            self._connection_ready_event = None
            self._connection_error = str(exc)
            return False

        # Register websocket event handlers
        self.connection.on(EventType.OPEN, self._on_open)
        self.connection.on(EventType.MESSAGE, self._handle_socket_message)
        self.connection.on(EventType.ERROR, self._handle_socket_error)
        self.connection.on(EventType.CLOSE, self._on_close)

        loop = asyncio.get_event_loop()
        self._listener_future = loop.run_in_executor(None, self.connection.start_listening)

        try:
            await asyncio.wait_for(self._connection_ready_event.wait(), timeout=10.0)
        except asyncio.TimeoutError:
            print("⏳ Timed out waiting for Deepgram connection to open")
            self._connection_error = "Connection timeout"
            return False
        finally:
            # Prevent future waits from hanging once the first event arrives
            self._connection_ready_event = None
    ...
    async def _shutdown_connection(self):
        ...
        if self.connection:
            try:
                self.connection.send_control(ListenV1ControlMessage(type="CloseStream"))
            except Exception:
                pass

        if self._listener_future:
            try:
                await asyncio.wait_for(self._listener_future, timeout=1.0)
            except asyncio.TimeoutError:
                pass
            except Exception:
                pass
        ...
        if self._connection_context:
            ...
            loop = asyncio.get_event_loop()
            try:
                await loop.run_in_executor(None, lambda: context.__exit__(None, None, None))
            except Exception:
                pass
    ...
    def _handle_socket_message(self, message: Any):
        ...
        try:
            if isinstance(message, ListenV1ResultsEvent):
                self._on_transcript(result=message)
            elif isinstance(message, ListenV1UtteranceEndEvent):
                self._on_utterance_end(result=message)
            ...
            now = time.time()
            if now - self._last_send_error_log > 1.0:
                print(f"⚠️  Failed to process Deepgram message: {dispatch_error}")
    ...
    def _handle_socket_error(self, error: Exception):
        ...
        if self._connection_ready_event and not self._connection_ready_event.is_set():
            self.event_loop.call_soon_threadsafe(self._connection_ready_event.set)
        self._on_error(error=error_message)
    ...
    async def start_listening(self) -> bool:
        ...
        try:
            ...
            if not connected or not self.connection_alive:
                error_detail = self._connection_error or "Unable to establish Deepgram connection"
                await self._emit_event(STTEventType.ERROR, {
                    "error": error_detail
                })
        ...
        while self.reconnect_attempts < self.max_reconnect_attempts and self.is_listening:
            ...
            try:
                ...
                if success:
                    ...
                    await self._emit_event(STTEventType.CONNECTION_OPENED, {
                        "message": "STT connection re-established"
                    })
                    self.reconnect_attempts = 0
                    self.is_reconnecting = False
                    return
        ...
        try:
            while self.is_listening and self.connection_alive:
                ...
                if self.connection:
                    ...
                    try:
                        self.connection.send_control(ListenV1ControlMessage(type="KeepAlive"))
                    except Exception as keepalive_error:
                        print(f"❌ Keepalive error: {keepalive_error}")
                        break
